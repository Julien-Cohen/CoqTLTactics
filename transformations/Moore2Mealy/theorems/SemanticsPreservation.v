...
Lemma star_step_commut_fw :
  forall i s r,
    ...
    MealySemantics.executeFromState
      ...
      (Moore2Mealy.convert_state s) i = Some r.
Proof.
...
Lemma star_step_commut_bw :
  forall i s' r,
    ...
    ( i= nil /\ r = nil ) \/
    exists s, 
      s'= Moore2Mealy.convert_state s /\ List.In (Moore.State s) m.(Model.modelElements) /\ MooreSemantics.executeFromState m s i = Some r.
Proof.
  ...
  }
  {
    right.
    rename s into s1'.
    assert (List.In (Mealy.State s1') (Semantics.execute Moore2Mealy.Moore2Mealy m).(Model.modelElements)).
    { apply MealySemantics.execute_in in STEP ; auto with wf. discriminate. }
...
Proof. 
  ...
  {
    cut (List.In (Moore.State s) m.(Model.modelElements)).
    {
      intro IN.
