...
Lemma tables_come_from_classes a b c : 
  In (TableElement a) (elements_proj (traceTrOnPiece Class2Relational_TUPLE_SP b [c])) ->
...
Lemma columns_come_from_attributes a b c d : 
  In (ColumnElement a) (elements_proj (traceTrOnPiece Class2Relational_TUPLE_SP b [c; d])) ->
...
Ltac show_origin :=
  ...
  match goal with 
   ...
   [ H : In (TableElement ?a) (elements_proj (traceTrOnPiece Class2Relational_TUPLE_SP ?b [?c])) |- _ ] =>
      destruct (tables_come_from_classes a b c H) as [newclassname TMP]; subst c

 | [ H : In (ColumnElement ?a) (elements_proj (traceTrOnPiece Class2Relational_TUPLE_SP ?b [?c; ?d])) |- _ ] =>
...
Lemma unify_table_class_lem :
  forall cm c ta,
    In (TableElement ta)
      (elements_proj (traceTrOnPiece Class2Relational_TUPLE_SP cm [ClassElement c])) ->
...
Ltac unify_table_class_tac H :=
  match type of H with
    In (TableElement ?ta) (elements_proj (traceTrOnPiece Class2Relational_TUPLE_SP _ [ClassElement ?c])) => apply unify_table_class_lem in H ; subst ta
  end.

Lemma unify_column_attribute_lem : 
  forall m a cl c, 
  In (ColumnElement c)
          (elements_proj (traceTrOnPiece Class2Relational_TUPLE_SP m
...
Ltac unify_column_attribute_tac H :=
  ...
  match type of H with 
    In (ColumnElement ?c)
      (elements_proj (traceTrOnPiece Class2Relational_TUPLE_SP _
         [AttributeElement ?a; ClassElement ?cl])) => 
